package nl.jorisdormans.graph
{
   ...
   public class Graph extends EventDispatcher
   {
      ...
      public function readXML(param1:XML) : void
      {
         ...
         while(_loc5_ < _loc4_)
         {
            if(_loc6_ = this.elements[_loc5_] as GraphConnection)
            {
               if(_loc6_.startId >= 0)
               {
      ...
      public function addXML(param1:XML) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            if(_loc7_ = this.elements[_loc6_] as GraphConnection)
            {
               if(_loc7_.startId >= 0)
               {
      ...
      public function addNode(param1:String, param2:Vector3D) : GraphNode
      {
         ...
         var _loc4_:GraphNode;
         (_loc4_ = new _loc3_.nodeClass()).symbol = _loc3_;
      ...
      {
         var _loc5_:GraphConnection;
         var _loc4_:GraphConnectionType;
         (_loc5_ = new _loc4_ = this.grammar.getConnectionType(param1).connectionClass()).type = _loc4_;
