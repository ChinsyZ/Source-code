package nl.jorisdormans.machinations.model
{
   ...
   public class Chart extends GroupBox
   {
      ...
      public function draw(param1:Graphics, param2:Number, param3:Number) : void
      {
         ...
         var _loc7_:Number;
         var _loc8_:Number = (_loc7_ = this._scaleY / (this._scaleY - this._negScaleY)) * 0.25;
         while(_loc8_ < 1)
         {
         ...
         param1.lineStyle();
         var _loc13_:* = this._scaleX.toString();
      ...
      public function toSVG(param1:XML) : void
      {
         ...
         var _loc11_:XML;
         (_loc11_ = DrawUtil.drawPathToSVG(_loc5_,_loc6_,"none",StringUtil.toColorStringSVG(0),1))["stroke-dasharray"] = "1,2";
         ...
         var _loc15_:* = this._scaleX.toString();
         if(Boolean(graph) && (graph as MachinationsGraph).timeMode == MachinationsGraph.TIME_MODE_TURN_BASED)
         {
            _loc15_ += " turns";
         }
      ...
      {
         if(this._tick > 0 && graph && !(graph as MachinationsGraph).running)
         {
            return;
         }
         ...
         var _loc3_:int = int(this._data.length - 1);
         while(_loc3_ >= 0)
         {
